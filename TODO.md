...
- [x] **#10 — Trainer parties are wrong**: Fixed. `scripts/sync-canonical-trainers.mjs` now auto-generates all outdoor route/forest trainers directly from pokered ASM source (correct class, Pokémon, levels, facing direction). Removed all hand-written duplicates and wrong entries. Re-run `node scripts/sync-canonical-trainers.mjs` to resync at any time.
