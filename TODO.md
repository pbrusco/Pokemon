...
1. **[ ] Fix the stitcher offsets**. Replace the propagating `from()` chain in
   `scripts/stitch-kanto.mjs` with a single hardcoded offset table that matches
   `KANTO_ZONE_OFFSETS` in `src/constants.ts`. Decide whether to move `ROUTE_21` to
   (118, 214) so south-of-Pallet shows Route 21 water (matches canonical).
   - Verify: world (118-137, 214-220) is empty (`T`) or Route 21 — never Fuchsia/Route 17.
   - Verify: world (109-128, 83-226) is no longer Route 17.
   - Verify: stitcher's printed offsets equal `KANTO_ZONE_OFFSETS` for every zone.

2. **[ ] Fix block classifier**. In `scripts/generate-overworld.mjs` `classifyQuadrant`,
   when the block is in `buildingBlocks` and the bottom-half quadrants classify as `W`/`D`,
   force the top-half to `W` (overriding the walkability-based `P` default). This recovers
   the visual roof.
   - Verify: block 0x38 → `[WW, WW]`, block 0x39 → `[WW, WW]`.
   - Verify: `pallet_town.json` house `W` spans 4 rows (e.g., rows 2-5), not 3.
   - Verify visually: Pallet houses show 2 rows of red roof + 2 rows of wall.

3. **[ ] Re-stitch & validate**. Run `node scripts/generate-overworld.mjs && node
   scripts/stitch-kanto.mjs`, then `bash .githooks/pre-commit`. Spot-check Pallet Town,
   Route 1, and walking south (should hit Route 21 water or empty trees, not Cycling Road).
