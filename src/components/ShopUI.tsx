...
import { ITEMS_DATABASE, SHOP_PRICES, SHOP_INVENTORY } from '../constants/items';

export const ShopUI = memo(({ onBuy, onClose, money, shopId }: { onBuy: (itemId: string) => void, onClose: () => void, money: number, shopId: string }) => {
  const shopItems = SHOP_INVENTORY[shopId] ?? ['POKEBALL', 'POTION']; 
  return (
    <motion.div
