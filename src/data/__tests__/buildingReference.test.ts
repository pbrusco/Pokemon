...
/**
 * Convert local zone coordinates to world coordinates on the overworld.
 */
...
/**
 * Check that a building's wall rectangle is solid W tiles on the overworld.
 */
...
/**
 * Check that a building has a door tile with a warp to the expected target.
 */
function expectDoorAndWarp(zone: string, b: BuildingFootprint) {
  if (b.doorX === null || b.doorY === null) return; // locked building, skip
...
/**
 * Check that the zone offset itself is valid.
 */
